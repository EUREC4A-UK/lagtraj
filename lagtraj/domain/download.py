...
def get_single_level_an(date, directories_dict, domain_dict, l_overwrite):
    ...
    this_filename = domain_filename_parse(
        date, prefix_str, directories_dict, domain_dict
    )
    ...
    file_exists = check_if_file_exists(
        date, prefix_str, directories_dict, domain_dict, this_dict
    )
    if l_overwrite or not (file_exists):
        p = multiprocessing.Process(
            target=retrieve_file, args=(this_repository, this_dict, this_filename,)
        )
        p.start()
