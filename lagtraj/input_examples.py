...
def print_available(input_types='all'):
    def _print_node(p):
        ...
        return OD([
            (p_node.name.replace('.yaml', ''), _print_node(p_node))
            for p_node in dirs
        ])

    tree = {
        'lagtraj://': _print_node(P_ROOT)
    }

    box_tr = LeftAligned(
        draw=drawing.BoxStyle(gfx=drawing.BOX_LIGHT)
    )
    print(box_tr(tree))


def main():
    print("The following domain/forcing/trajectory definitions are"
    ...
    print("\n")
    print("To use for example the `eurec4a_north_atlantic` domain definition\n"
